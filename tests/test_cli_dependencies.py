"""Tests for ``vyne.cli.dependencies`` — structure-preserving PEP 508 aware
dependency editing.

Coverage:
- Tool tables before/after [project]
- Case/dash/underscore normalization and unrelated prefixes
- URL/local requirements
- Comments, multiline arrays, trailing commas
- Malformed/non-string entries
...
class DependencyEditingTests(unittest.TestCase):
    """CLI-01: Structure-preserving TOML + PEP 508 dependency editing."""
    ...
    # -- no-op when vyne already present ------------------------------------

    def test_noop_existing_vyne_simple(self):
        text = '[project]\ndependencies = ["vyne"]\n'
        self.assertEqual(ensure_vyne_dependency(text, "vyne>=1.0"), text)

    def test_noop_existing_vyne_with_extras(self):
        text = '[project]\ndependencies = ["requests", "vyne[extra]"]\n'
        self.assertEqual(ensure_vyne_dependency(text, "vyne>=1.0"), text)
    ...
    def test_noop_existing_vyne_case_normalized(self):
        text = '[project]\ndependencies = ["Vyne>=1.0"]\n'
        self.assertEqual(ensure_vyne_dependency(text, "vyne>=2.0"), text)

    def test_noop_existing_vyne_underscore_normalized(self):
        text = '[project]\ndependencies = ["vy_ne>=1.0"]\n'
        # "vy_ne" canonicalizes to "vy-ne", not "vyne"
        # So this should NOT match and should insert
        result = ensure_vyne_dependency(text, "vyne>=1.0")
        import tomllib
        parsed = tomllib.loads(result)
        deps = parsed["project"]["dependencies"]
    ...
    # -- vyness/vyne-core false positives -----------------------------------
    ...
    # -- multiline arrays and trailing commas -------------------------------
    ...
    def test_rejects_non_array_dependencies(self):
